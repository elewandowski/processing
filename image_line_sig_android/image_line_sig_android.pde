...
void setup(){
  size(1024, 768);
  imgNum = 8;
  videoFolderName = generateVideoFolderName();
...
//          + " imgSize: " + imgSize);
  ...
  for(int i=0; i<numOfRows; i++) {
    int mouseXMul = (int) ((float)mouseX/width * 50);
    int rowStart = ((rowNum + i * mouseXMul) * img.width) % imgSize;
  ...
  if(savingFrame) saveFrame("video/" + videoFolderName + "/screen-#####.png");
}
